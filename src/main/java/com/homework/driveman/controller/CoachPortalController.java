...
/**
 * 教练端个人业务控制器
 * 统一前缀 /coach，处理教练登录后的个人操作（查看学员、录入学时、工作量等）
 */
@Tag(name = "教练端业务")
@RestController
...
public class CoachPortalController {
    ...
    /**
     * 从请求头中解析 token，获取当前登录教练的 coachId
     * 若 token 无效或用户不是教练，抛出异常
     */
    ...
    public JsonResult<List<StudentInfoVO>> listMyStudents(HttpServletRequest request) {
        ...
        List<StudentInfoVO> students = coachService.getMyStudents(coachId);
