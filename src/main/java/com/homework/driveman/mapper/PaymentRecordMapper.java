...
public interface PaymentRecordMapper extends BaseMapper<PaymentRecord> {
    ...
    @Select("<script>" +
            "SELECT p.*, u.real_name AS student_name, u.phone AS student_phone, u.license_type " +
            "FROM payment_record p " +
            "JOIN user u ON p.student_id = u.user_id " +
            "WHERE p.status = 0 AND p.is_deleted = 0 AND u.is_deleted = 0 " +
            "  <if test='keyword != null and keyword != \"\"'>AND u.real_name LIKE CONCAT('%', #{keyword}, '%')</if> " +
            "  <if test='phone != null and phone != \"\"'>AND u.phone LIKE CONCAT('%', #{phone}, '%')</if> " +
            "  <if test='licenseType != null and licenseType != \"\"'>AND u.license_type = #{licenseType}</if> " +
            "  <if test='bizType != null and bizType != \"\"'>AND p.biz_type = #{bizType}</if> " +
            "ORDER BY p.create_time DESC" +
            "</script>")
